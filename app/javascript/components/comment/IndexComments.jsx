...
function IndexComments({post={}}) {
  ...
  const fetchComments = async () => {
    try {
      ...
      const { data } = response;
      const orderCommentss = data.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
      setComments(orderCommentss);
    } catch (error) {
            ...
            <div className='overflow-y-auto'>
              {comments && comments.length > 0 ? (
                comments.map((comment) => (
                  <div key={comment.id}>
                    <div className="flex min-w-96 justify-start p-4">
                      ...
                      <div className="min-w-72">
                        <div className="flex-1">
                          <p className="w-full pl-2">{comment.user.name}</p>
                    ...
                    <div className='flex justify-end items-center'>
                      <p className='flex-none text-xs mr-6'>{(comment.created_at).split('T')[0]}</p>
