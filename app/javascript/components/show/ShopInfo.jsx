...
function ShopInfo({ shopId = '' }) {
  ...
  const fetchShop = async () => {
    try {
      ...
      const { data } = response;
      setShop(data); // 取得したショップ情報をstateに設定
      const srcRegex = /src="([^"]*)"/;
      const mapIflame = data.map_iflame.match(srcRegex);
      if (mapIflame) { // マッチした場合のみ処理を行う
        setIflame(mapIflame[1]);
      }
    } catch (error) {
      alert('リクエストエラー');
      console.error('shop情報の取得エラー:', error);
    }
  };
      ...
      <div className="container mx-auto">
        { shop !== null ? (
          <div className="text-sm sm:text-base">
            <div className="mt-4 flex w-full border border-black">
                    ...
                    <table className="flex size-full flex-col bg-slate-200">
                      <thead className="flex">
                        <tr className="flex flex-1">
                          <th className="flex-1 border border-black sm:text-3xl ">{shop.name}</th>
                        </tr>
                      </thead>
